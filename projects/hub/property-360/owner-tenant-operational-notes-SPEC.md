...
**What I cannot verify by reading this codebase, and Peter must confirm directly, before this path goes live (not before development — counsel's own distinction, Section 5 of the opinion):**
- Whether Rincon's actual Anthropic account/agreement includes contractual terms matching counsel's specific list: no independent training on Rincon's content, no profiling, no sale, defined retention/deletion, confidentiality, appropriate security. A standard API call is a **technical** fact (verified above); whether the **commercial terms** governing that specific account satisfy "contracted processor, no independent use" is a business/legal question this spec cannot answer from source code — it depends on which Anthropic plan/agreement Rincon is actually on, and whether a Data Processing Agreement or equivalent has been signed. Flagged for Peter to confirm directly with Anthropic (or Rincon's own account terms), not resolved here.
...
3. **The AI-vendor contractual confirmation** (Section 8) — a real business/contract item for Peter to close out directly with Anthropic (or against Rincon's existing agreement), not something this spec or any code review can resolve.
...
5. **Mason should write the actual staff-facing content guidance** referenced in Sections 6 and 7 (the owner-instruction-rejection template, the fact-vs-characterization examples for the soft warning), **plus the new Tier 2/3 access-use policy text** (Section 3) shown in the acknowledgment gate — this spec designs where that guidance plugs in, not the guidance itself.
